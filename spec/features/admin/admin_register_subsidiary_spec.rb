...
feature 'Admin register subsidiary' do
  ...
  scenario 'successfully and generates addon prices' do
    user = create(:user, role: :admin)
    create(:category, name: 'A', daily_rate: 51.0, car_insurance: 52.0,
                      third_party_insurance: 53.0)
    create(:category, name: 'B', daily_rate: 54.0, car_insurance: 55.0,
                      third_party_insurance: 56.0)
    create(:addon, name: 'Bebê conforto', description: 'Cadeira paara bebê',
                   standard_daily_rate: 32.0)
    create(:addon, name: 'GPS', description: 'GPS Garmin',
                   standard_daily_rate: 26.0)
    ...
    expect(page).to have_content('R$ 32,0')
    expect(page).to have_content('R$ 26,0')
  end
